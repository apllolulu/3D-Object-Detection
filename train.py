...
parser.add_argument('-i', '--max-epoch', type=int, nargs='?', default=168,#160
                    help='max epoch')
parser.add_argument('-n', '--tag', type=str, nargs='?', default='pre_trained_car',#default
                    help='set log tag')
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(start_epoch, args.max_epoch):
                ...
                for batch in iterate_data(train_dir, shuffle=True, aug=True, is_testset=False, batch_size=args.single_batch_size * cfg.GPU_USE_COUNT, multi_gpu_sum=cfg.GPU_USE_COUNT):
                    ...
                    print('loss: {:.4f} reg_loss: {:.4f} cls_loss: {:.4f} cls_pos_loss: {:.4f} cls_neg_loss: {:.4f} forward time: {:.4f} batch time: {:.4f}\n'.format( ret[0], ret[1], ret[2], ret[3], ret[4], forward_time, batch_time))
